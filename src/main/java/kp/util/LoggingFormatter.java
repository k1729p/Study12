...
public class LoggingFormatter extends Formatter {
	...
	public String format(LogRecord logRecord) {
		...
		stringBuilder.append(logRecord.getLevel().toString().substring(0, 3));
		...
		stringBuilder.append(super.formatMessage(logRecord)).append(EOL);
		...
		if (thr1 != null) {
			...
			stringBuilder.append("   ");
			stringBuilder.append(thr1.toString()).append(EOL);
			final StackTraceElement[] stTrElem = thr1.getStackTrace();
			for (int i = 0; i < stTrElem.length; i++) {
				stringBuilder.append("      ");
				stringBuilder.append(stTrElem[i].toString()).append(EOL);
			}
		}
