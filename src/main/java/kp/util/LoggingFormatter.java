...
public class LoggingFormatter extends Formatter {
    ...
    public String format(LogRecord logRecord) {
        ...
        stringBuilder.append(logRecord.getLevel().toString(), 0, 3).append(" ")
                .append(logRecord.getLongThreadID()).append(" ")
                .append(dateFormat.format(new Date(logRecord.getMillis()))).append(" ");
        if (SHOW_CLASS) {
            String className = logRecord.getSourceClassName();
            className = className.substring(className.lastIndexOf('.') + 1);
            stringBuilder.append(className).append(".");
            if (SHOW_METHOD) {
                stringBuilder.append(logRecord.getSourceMethodName()).append("(): ");
            }
        }
        ...
        if (thr1 != null) {
            ...
            stringBuilder.append("   ").append(thr1).append(System.lineSeparator());
            for (StackTraceElement stackTraceElement : thr1.getStackTrace()) {
                stringBuilder.append("      ").append(stackTraceElement).append(System.lineSeparator());
            }
        }
